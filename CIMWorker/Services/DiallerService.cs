...
namespace CIMWorker.Services
{
   ...
   public interface IDiallerService
   {
      ...
      Task<Dictionary<int, int>> GetQueueSourceList(string ConnString);
   ...
   public class DiallerService : IDiallerService
   {
      ...
      public async Task<Dictionary<int, int>> GetQueueSourceList(string ConnString)
      {
         Dictionary<int, int> model = new Dictionary<int, int>();
         DataTable TempData = new DataTable();
         string query = @"SELECT [ID],[SOURCEID] FROM [PREP].[PCO_OUTBOUNDQUEUE]";
