...
namespace CIMWorker.Controllers
{
    public class DataQueueController
    {
        ...
        private Dictionary<int, int> QueueSourceList;
        ...
        public async Task ProcessRecord(PhoneQueue record, Dictionary<int, string> LoadStatusList, Dictionary<int, int> QueueSourceList)
        {
            try
            {
                if (record.Command == "callback")
                {
                    ...
                    bool InPresence = QueueSourceList.ContainsValue(record.SourceID);
                    if (InPresence)
                    {
        ...
        public async Task<bool> IsSameService(PhoneQueue data)
        {
            ...
            try
            {
                string query = @"SELECT TOP 1 [SERVICEID] FROM [PREP].[PCO_OUTBOUNDQUEUE] WHERE [SOURCEID] = @SOURCEID";
                ...
                if (dataOut.SERVICEID == data.ServiceID)
                {
                    return true;
                }
            }
                ...
                string query = @"INSERT INTO [PREP].[PCO_OUTBOUNDQUEUE]
                           ...
                           (
                              @SERVICEID,LEFT(@NAME,40),@PHONE,@SOURCEID,@STATUS,@SCHEDULETYPE,@SCHEDULEDATE,@LOADID,@PRIORITY,NULL,
	                           @PHONE1,@PHONE2,@PHONE3,@PHONE4,@PHONE5,@PHONE6,@PHONE7,@PHONE8,@PHONE9,@PHONE10,
                              @PHONEDESC1,@PHONEDESC2,@PHONEDESC3,@PHONEDESC4,@PHONEDESC5,@PHONEDESC6,@PHONEDESC7,@PHONEDESC8,@PHONEDESC9,@PHONEDESC10,
                              @COMMENTS,@CUSTOMDATA1,(SELECT [DESCRIPTION] FROM [PREP].[PCO_LOAD] WHERE [SERVICEID] = @SERVICEID AND [LOADID] = @LOADID),@CUSTOMDATA3,GETDATE()
                           )";
        ...
        public async Task RemoveFromPresence(PhoneQueue data)
        {
            try
            {
                string query = @"DELETE FROM [PREP].[PCO_OUTBOUNDQUEUE] WHERE [SERVICEID] BETWEEN 141 AND 162 AND [SOURCEID] = @SourceID";
                using IDbConnection connection = new SqlConnection(_dataService.GetConnectionString("Presence"));
                {
        ...
        public async Task ProcessCallback(PhoneQueue data, int LoadStatus)
        {
            try
            {
                string query = @"UPDATE [PREP].[PCO_OUTBOUNDQUEUE]
                                SET CAPTURINGAGENT = @CAPTURINGAGENT, SCHEDULEDATE = @SCHEDULEDATE, STATUS = @STATUS, LOADID = @LOADID
                                WHERE SERVICEID = @SERVICEID AND LOADID IN (@LOADID, @LOADID + 9000) AND SOURCEID = @SOURCEID";
                using IDbConnection connection = new SqlConnection(_dataService.GetConnectionString("Presence"));
                {
