...
namespace CIMWorker.Controllers
{
    ...
    public class ImportDatabaseController
    {
        ...
        public async Task<bool> MasterAsync()
        {
            ...
            List<InPhoneCallActivity> collection = new List<InPhoneCallActivity>();// await _dataService.SelectMany<InPhoneCallActivity, dynamic>("EXECUTE [dbo].[spGetNewPhoneCallsv2]", new { });
            List<InMaxStatus> inMaxStatuses =  await _dataService.SelectMany<InMaxStatus, dynamic>("EXECUTE [dbo].[spGetNewMaximums]", new { });
            List<InPhoneCallActivity> collectionUnreachable = await _dataService.SelectMany<InPhoneCallActivity, dynamic>("EXECUTE [dbo].[spCreateUnreachable]", new { });
            ...
            //await _logService.InfoAsync($"Import Database : Unreachable Query Returned {collectionUnreachable.Count()}");
            //if(collectionUnreachable.Count > 0)
            //{
            //    foreach(var item in collectionUnreachable)
            //    {
            //        item.subject = "Unreachable";
            //        var output = item.GetOutCall();
            //        var data = new StringContent(JsonConvert.SerializeObject(output), Encoding.UTF8, "application/json");
            //        using var client = await GetHttpClient();
            //        try
            ...
            //            if (check.IsSuccessStatusCode)
            //            {
            //                await InsertUnreachableSent(item.ID);
            //            }
            //        }
            //        catch (Exception ex)
            //        {
        ...
        #region [ Insert into OBLSent ]
        private async Task InsertUnreachableSent(int IDin)
        {
            try
            {
                await _dataService.InsertSingle<dynamic, dynamic>(@"INSERT INTO [dbo].[UnreachableSent]
                                                                                       ([UnreachableOBLID])
                                                                                 VALUES
                                                                                       (@OBLID)", new { OBLID = IDin });
            }
